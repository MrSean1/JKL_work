...
url = 'http://api.bxx.com/api/data_NK_HS-YM/login.jhtml?userCode=15000000001&password=bxx001&mastKey=2cb8aabb1ea2c2728afa8c9b3c675f27'
...
#
# # all_token = []
# # data_NK_HS-YM = pd.read_table('test_account.txt')
# # access_key_id = list(data_NK_HS-YM['access_key_id'])
# # secret_key = list(data_NK_HS-YM['access_key_secret'])
# # thp_token = []
...
#
# data_NK_HS-YM = pd.read_table('tokens.txt', header=None)
# all_token = list(data_NK_HS-YM.iloc[:, 0])
# while True:
#     # thp_order_buy = []
#     # for i in range(500):
...
async def main_accounts():
    async with aiohttp.ClientSession() as session:
        ...
        await asyncio.wait(tasks)
        return [task.result()['data_NK_HS-YM']['assertList'][0:2] for task in tasks]
